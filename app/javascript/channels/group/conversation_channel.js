...
function subToGroupConversationChannel(id) {
  window.App['group_conversation_' + id]  = consumer.subscriptions.create(
      {
...
$(document).on('submit', '.send-group-message', function(e) {
  ...
  window.App['group_conversation_' + id].send_message(message_values);
});
...
$(document).on('click', '.conversation-window, .group-conversation', function(e) {
  ...
  if (unseen_by_user) {
      var conv_id = $(this).find('.card').attr('data-gconversation-id');
      ...
      if (window.App["group_conversation_" + conv_id]) {
        window.App["group_conversation_" + conv_id].set_as_seen(conv_id);
      } else {
        console.warn(`Subscription for group_conversation_${conv_id} not found`);
      }
     //window.App['group_conversation_' + conv_id].set_as_seen(conv_id);
  }
});
