...
import {calculateContactRequests } from "../contact_requests.js";

window.App=window.App||{};

window.App.notification=consumer.subscriptions.create("NotificationChannel", {
  connected() {
  ...
  received(data) {
   ...
   if (data['notification'] == 'contact-request-received') {
     var conversation_window = $('body').find('[data-pconversation-user-name="' + data["sender_name"] + '"]');
     var has_no_contact_requests = $('#contacts-requests ul').find('.no-requests');
     var contact_request = data['contact_request'];
